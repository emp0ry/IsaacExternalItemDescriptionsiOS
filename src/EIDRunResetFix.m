...
@interface NSObject (EIDRunResetFix)
- (void)eid_runreset_updateMenuModeForGameplay:(BOOL)gameplayActive;
@end

@implementation NSObject (EIDRunResetFix)
- (void)eid_runreset_updateMenuModeForGameplay:(BOOL)gameplayActive {
...
__attribute__((constructor)) static void EIDInstallRunResetFix(void) {
    dispatch_async(dispatch_get_main_queue(), ^{
        Class cls = NSClassFromString(@"EIDOverlayController");
        if (!cls) return;
        SEL replacementSEL = @selector(eid_runreset_updateMenuModeForGameplay:);
        ...
        Method original = class_getInstanceMethod(cls, NSSelectorFromString(@"updateMenuModeForGameplay:"));
